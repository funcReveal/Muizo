...
const normalizePlaylistItems = (items: PlaylistItem[]) =>
  items.map((item) => {
...
export const RoomProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchCollections = useCallback(async () => {
    ...
    try {
      const run = async (token: string, allowRetry: boolean) => {
        const url = buildWorkerUrl("collections");
        if (!url) {
  ...
  const loadCollectionItems = useCallback(
    async (collectionId: string) => {
      ...
      try {
        const run = async (token: string, allowRetry: boolean) => {
          const url = buildWorkerUrl(`collections/${collectionId}/items`);
          if (!url) {
            throw new Error("尚未設定收藏庫 API 位置 (WORKER_API_URL)");
          }
          url.searchParams.set("pageSize", String(DEFAULT_PAGE_SIZE));
          ...
          if (res.ok) {
            ...
            setPlaylistStage("preview");
            setPlaylistLocked(true);
            setLastFetchedPlaylistId(collectionId);
            setPlaylistUrl("");
