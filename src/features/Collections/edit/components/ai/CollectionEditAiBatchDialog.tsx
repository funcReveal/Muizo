      ...
      slotProps={{
        paper: {
          sx: {
            ...
            background:
              "radial-gradient(circle at 14% 0%, rgba(34,211,238,0.15), transparent 34%), radial-gradient(circle at 100% 18%, rgba(251,191,36,0.10), transparent 30%), linear-gradient(180deg, rgba(8,17,31,0.99), rgba(3,7,18,0.99))",
      ...
      <div className="flex h-full min-h-0 flex-col">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-cyan-300/12 px-4 py-3 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <span className="hidden h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-cyan-100/16 bg-cyan-300/10 text-cyan-100 sm:inline-flex">
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          {isIdle ? (
            <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden lg:grid-cols-[300px_minmax(0,1fr)] lg:grid-rows-none">
              <aside className="border-b border-cyan-300/12 bg-slate-950/28 px-4 py-3 lg:border-b-0 lg:border-r lg:px-5 lg:py-5">
                <div className="flex items-center justify-between gap-3 lg:block">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-[var(--mc-text-muted)]">
                      每批產生 prompt、貼回 JSON 後再套用。
                    </div>
                  </div>

                  <div className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs font-semibold text-slate-200 lg:mt-3 lg:inline-flex">
                  ...
                  <section className="rounded-2xl border border-[var(--mc-border)] bg-[var(--mc-surface)]/60 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.035)]">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-[var(--mc-text-muted)]">
                          支援純 JSON，也支援包在 ```json code block 內的格式。
                        </div>
                      </div>
        ...
        <footer className="shrink-0 border-t border-cyan-200/14 bg-[linear-gradient(90deg,rgba(8,47,73,0.16),rgba(15,23,42,0.72)_44%,rgba(120,53,15,0.1))] px-4 py-3 shadow-[0_-18px_48px_-42px_rgba(34,211,238,0.75)] sm:px-6">
