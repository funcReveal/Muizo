...
const CareerCollectionRanksTab: React.FC<CareerCollectionRanksTabProps> = ({
  ...
  return (
    <div className="mt-5 space-y-4">
      <section className="rounded-[24px] border border-[var(--mc-border)] bg-[linear-gradient(180deg,rgba(20,17,13,0.96),rgba(8,7,5,0.98))] p-5 shadow-[0_18px_38px_-28px_rgba(0,0,0,0.72)]">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--mc-text-muted)]">
              這裡專門看你在各題庫的表現。重點不只是目前名次，還要能看到最近名次有沒有上升或下降。
            </p>
          </div>
      ...
      <section className="overflow-hidden rounded-[24px] border border-[var(--mc-border)] bg-[linear-gradient(180deg,rgba(20,17,13,0.96),rgba(8,7,5,0.98))] shadow-[0_18px_38px_-28px_rgba(0,0,0,0.72)]">
        <div className="grid grid-cols-[minmax(0,2fr)_100px_100px_110px_120px_100px_120px] gap-3 border-b border-[var(--mc-border)] px-5 py-4 text-xs font-semibold tracking-[0.12em] text-[var(--mc-text-muted)]">
          <div>題庫</div>
          <div>最佳名次</div>
          <div>目前名次</div>
          <div>Δ 變動</div>
          ...
          <div className="divide-y divide-[var(--mc-border)]">
            {items.map((item) => (
              ...
              >
                ...
                <div className={`font-semibold ${deltaClassName(item.delta)}`}>
